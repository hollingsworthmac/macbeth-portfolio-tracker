...
export default function NewTransactionPage() {
  ...
  const onSubmit = (e: React.FormEvent) => {
    e.preventDefault()
    if (!accountId || !date || !action || !symbol) {
      setMsg('Account, Date, Action and Symbol are required')
      return
    }
  ...
  return (
    <div>
      ...
      <form onSubmit={onSubmit} style={{ display:'grid', gap: 8, maxWidth: 520 }}>
        ...
        <input type="number" step="any" placeholder="Quantity" value={quantity}
               onChange={e=>setQuantity(Number(e.target.value))} />
        <input type="number" step="any" placeholder="Price" value={price}
